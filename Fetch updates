“””
Iran / Regional Tensions — Automated Intelligence Tracker
Fetches RSS feeds, extracts structured data via Claude API, saves to entries.json
“””
...
# Keywords to filter relevant articles (case-insensitive)
...
“persian gulf”, “idf”, “mossad”, “revolution guard”,
]
...
Extract ONLY factual data from official or verified sources. No analysis, no opinion, no commentary.

Rules:

- Use EXACT wording from source for the excerpt field. Do not paraphrase.
- If multiple distinct events exist in the text, return multiple objects.
...
- date: ISO 8601 format YYYY-MM-DDTHH:MM — if no time use T00:00
- location: city/region/country or “Unknown”
- headline: max 12 words, factual only
- relevance: true if related to Iran/regional tensions/Middle East military activity, false otherwise
...
for source in SOURCES:
    ...
    print(f"  {len(new_items)} new relevant items (of {len(items)} fetched)")
...
if not new_items_total:
    print("\n✓ No new relevant items this run.")
    save_json(SEEN_FILE, list(seen_ids))
    return

# Process in batches of 10
print(f"\n→ Sending {len(new_items_total)} items to Claude for extraction...")
extracted = []
for i in range(0, len(new_items_total), 10):
    ...
    print(f"  Batch {i//10 + 1}: extracted {len(result)} entries")
    time.sleep(2)
...
print(f"\n✓ Done. {len(timestamped)} new entries added. Total: {len(all_entries)}")
```
